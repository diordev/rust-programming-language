...
[Listing 5-1](#listing-5-1): A `User` struct definition
...
[Listing 5-2](#listing-5-2): Creating an instance of the `User` struct
...
[Listing 5-3](#listing-5-3): Changing the value in the `email` field of a `User` instance
...
[Listing 5-4](#listing-5-4): A `build_user` function that takes an email and username and returns a `User` instance
...
[Listing 5-5](#listing-5-5): A `build_user` function that uses field init shorthand because the `username` and `email` parameters have the same name as struct fields
...
[Listing 5-6](#listing-5-6): Creating a new `User` instance using all but one of the values from `user1`
...
[Listing 5-7](#listing-5-7): Using struct update syntax to set a new `email` value for a `User` instance but to use the rest of the values from `user1`
