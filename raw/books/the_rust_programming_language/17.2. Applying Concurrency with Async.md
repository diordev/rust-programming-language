...
source: "https://doc.rust-lang.org/stable/book/ch17-02-concurrency-with-async.html"
author:
published:
created: 2026-05-07
description:
...
[Listing 17-6](#listing-17-6): Creating a new task to print one thing while the main task prints something else
...
[Listing 17-7](#listing-17-7): Using `await` with a join handle to run a task to completion
...
[Listing 17-8](#listing-17-8): Using `trpl::join` to await two anonymous futures
...
[Listing 17-9](#listing-17-9): Creating an async channel and assigning the two halves to `tx` and `rx`
...
[Listing 17-10](#listing-17-10): Sending and receiving multiple messages over the async channel and sleeping with an `await` between each message
...
In Listing 16-10, we used a `for` loop to process all the items received from a synchronous channel. Rust doesn’t yet have a way to use a `for` loop with an *asynchronously produced* series of items, however, so we need to use a loop we haven’t seen before: the `while let` conditional loop. This is the loop version of the `if let` construct we saw back in the [“Concise Control Flow with `if let` and `let...else` ”](https://doc.rust-lang.org/stable/book/ch06-03-if-let.html) section in Chapter 6. The loop will continue executing as long as the pattern it specifies continues to match the value.
...
[Listing 17-11](#listing-17-11): Separating `send` and `recv` into their own `async` blocks and awaiting the futures for those blocks
...
[Listing 17-12](#listing-17-12): A revision of the code from Listing 17-11 that correctly shuts down when complete
...
[Listing 17-13](#listing-17-13): Using multiple producers with async blocks
