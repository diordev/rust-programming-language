...
source: "https://rust-for-backend-developers.github.io/rust-basics/generics.html"
author:
published:
created: 2026-05-16
...
| ```rust fn make_holder<T>(v: T) -> Holder<T> {     Holder { v: v } } ``` | ```rust let h1 = make_holder(5); let h2 = make_holder(true); ``` |
| --- | --- |
| ↓ | ↓ |
| ```rust fn make_holder_i32(v: i32) -> Holder_i32 {     Holder_i32 { v: v } }  fn make_holder_bool(v: bool) -> Holder_bool {     Holder_bool { v: v } } ``` | ```rust let h1 = make_holder_i32(5); let h2 = make_holder_bool(true); ``` |
...
| Трэйт с генерик тип-аргументом ```rust trait Трэйт<A> {     ... }  struct S {     ... }  impl Трэйт<i32> for S {     ...  } ``` | Трэйт с ассоциированным типом ```rust trait Трэйт {     type A;     ... }  struct S {     ... }  impl Трэйт for S {     type A = i32;     ... } ``` |
| --- | --- |
...
На этом моменте вам может показаться, что это ограничение типов по реализуемому ими трэйту очень похоже на передачу аргументов по `impl Трэйт`, с которым мы познакомились в главе [Трэйты / Статическая диспетчеризация](https://rust-for-backend-developers.github.io/rust-basics/traits.html#static-dispatching). Ведь по сути, `impl Трэйт` тоже может быть заменён только на тип, который этот трэйт реализует.

| генерик с границей  ```rust fn my_func<T: Трэйт>(v: T) -> ... {     ... } ``` | `impl Трэйт`  ```rust fn my_func(v: impl Трэйт) -> ... {     ... } ``` |
| --- | --- |
...
Чтобы продемонстрировать это, давайте перепишем пример, который мы использовали в главе [Трэйты / Статическая диспетчеризация](https://rust-for-backend-developers.github.io/rust-basics/traits.html#static-dispatching), с использованием границ генерика.
...
| Без where блока ```rust fn my_func<A: Трэйт1, B: Трэйт2>(     аргумент1: A, аргумент2: B, ) -> Тип {     ...  } ``` | С where блоком ```rust fn my_func<A, B>(     аргумент1: A, аргумент2: B, ) -> Тип where A: Трэйт1, B: Трэйт2 {     ... } ``` |
| --- | --- |
...
| генерик с границей  ```rust struct Holder<T: Clone> {     v: T } ``` | блок `where`  ```rust struct Holder<T> where T: Clone {     v: T } ``` |
| --- | --- |
