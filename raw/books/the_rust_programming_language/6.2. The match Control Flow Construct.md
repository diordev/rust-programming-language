...
source: "https://doc.rust-lang.org/stable/book/ch06-02-match.html"
author:
published:
created: 2026-05-06
description:
...
[Listing 6-3](#listing-6-3): An enum and a `match` expression that has the variants of the enum as its patterns
...
[Listing 6-4](#listing-6-4): A `Coin` enum in which the `Quarter` variant also holds a `UsState` value
...
[Listing 6-5](#listing-6-5): A function that uses a `match` expression on an `Option<i32>`
...
There’s more about patterns and matching that we’ll cover in [Chapter 19](https://doc.rust-lang.org/stable/book/ch19-00-patterns.html). For now, we’re going to move on to the `if let` syntax, which can be useful in situations where the `match` expression is a bit wordy.