...
source: "https://doc.rust-lang.org/stable/book/ch20-03-advanced-types.html"
author:
published:
created: 2026-05-07
description:
...
[Listing 20-25](#listing-20-25): Using a long type in many places
...
[Listing 20-26](#listing-20-26): Introducing a type alias, `Thunk`, to reduce repetition
...
[Listing 20-27](#listing-20-27): A `match` with an arm that ends in `continue`

At the time, we skipped over some details in this code. In [“The `match` Control Flow Construct”](https://doc.rust-lang.org/stable/book/ch06-02-match.html#the-match-control-flow-construct) section in Chapter 6, we discussed that `match` arms must all return the same type. So, for example, the following code doesn’t work:
...
We can combine `str` with all kinds of pointers: for example, `Box<str>` or `Rc<str>`. In fact, you’ve seen this before but with a different dynamically sized type: traits. Every trait is a dynamically sized type we can refer to by using the name of the trait. In the [“Using Trait Objects to Abstract over Shared Behavior”](https://doc.rust-lang.org/stable/book/ch18-02-trait-objects.html#using-trait-objects-to-abstract-over-shared-behavior) section in Chapter 18, we mentioned that to use traits as trait objects, we must put them behind a pointer, such as `&dyn Trait` or `Box<dyn Trait>` (`Rc<dyn Trait>` would work too).
