...
source: "https://doc.rust-lang.org/stable/book/ch07-03-paths-for-referring-to-an-item-in-the-module-tree.html"
author:
published:
created: 2026-05-06
description:
...
[Listing 7-3](#listing-7-3): Calling the `add_to_waitlist` function using absolute and relative paths
...
[Listing 7-4](#listing-7-4): Compiler errors from building the code in Listing 7-3
...
[Listing 7-5](#listing-7-5): Declaring the `hosting` module as `pub` to use it from `eat_at_restaurant`
...
[Listing 7-6](#listing-7-6): Compiler errors from building the code in Listing 7-5
...
[Listing 7-7](#listing-7-7): Adding the `pub` keyword to `mod hosting` and `fn add_to_waitlist` lets us call the function from `eat_at_restaurant`.
...
[Listing 7-8](#listing-7-8): Calling a function using a relative path starting with `super`
...
[Listing 7-9](#listing-7-9): A struct with some public fields and some private fields
...
[Listing 7-10](#listing-7-10): Designating an enum as public makes all its variants public.
