...
source: "https://doc.rust-lang.org/stable/book/ch21-02-multithreaded.html"
author:
published:
created: 2026-05-07
description:
...
[Listing 21-10](#listing-21-10): Simulating a slow request by sleeping for five seconds
...
[Listing 21-11](#listing-21-11): Spawning a new thread for each stream
...
[Listing 21-12](#listing-21-12): Our ideal `ThreadPool` interface
...
[Listing 21-13](#listing-21-13): Implementing `ThreadPool::new` to panic if `size` is zero
...
[Listing 21-14](#listing-21-14): Creating a vector for `ThreadPool` to hold the threads
...
[Listing 21-15](#listing-21-15): Modifying `ThreadPool` to hold `Worker` instances instead of holding threads directly
...
Note: If the operating system can’t create a thread because there aren’t enough system resources, `thread::spawn` will panic. That will cause our whole server to panic, even though the creation of some threads might succeed. For simplicity’s sake, this behavior is fine, but in a production thread pool implementation, you’d likely want to use [`std::thread::Builder`](https://doc.rust-lang.org/stable/std/thread/struct.Builder.html) and its [`spawn`](https://doc.rust-lang.org/stable/std/thread/struct.Builder.html#method.spawn) method that returns `Result` instead.
...
[Listing 21-16](#listing-21-16): Modifying `ThreadPool` to store the sender of a channel that transmits `Job` instances
...
[Listing 21-17](#listing-21-17): Passing the receiver to each `Worker`
...
[Listing 21-18](#listing-21-18): Sharing the receiver among the `Worker` instances using `Arc` and `Mutex`
...
[Listing 21-19](#listing-21-19): Creating a `Job` type alias for a `Box` that holds each closure and then sending the job down the channel
...
[Listing 21-20](#listing-21-20): Receiving and executing the jobs in the `Worker` instance’s thread
...
[Listing 21-21](#listing-21-21): An alternative implementation of `Worker::new` using `while let`
