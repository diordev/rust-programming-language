...
[Listing 15-14](#listing-15-14): A `CustomSmartPointer` struct that implements the `Drop` trait where we would put our cleanup code
...
[Listing 15-15](#listing-15-15): Attempting to call the `drop` method from the `Drop` trait manually to clean up early
...
[Listing 15-16](#listing-15-16): Calling `std::mem::drop` to explicitly drop a value before it goes out of scope
