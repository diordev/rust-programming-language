---
title: Any
source: https://rust-for-backend-developers.github.io/dive-deeper/any.html
author:
published:
created: 2026-05-16
...
Благодаря [трэйт-объектам](https://rust-for-backend-developers.github.io/rust-basics/traits.html#dynamic_dispatching), мы в любой момент можем сделать “upcast”, то есть начать общаться с типом через его трэйт, абстрагируясь от того, какой конкретно тип скрыт за трэйт-объектом.
...
Как видно из объявления, абсолютно для любого типа, реализующего `'static`, определена реализация `Any`, которая просто получает `TypeId` типа. По сути, это то же самое, что мы делали методом *exact\_type* в примере выше.
