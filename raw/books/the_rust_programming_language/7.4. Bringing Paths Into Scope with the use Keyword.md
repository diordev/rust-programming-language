...
[Listing 7-11](#listing-7-11): Bringing a module into scope with `use`
...
[Listing 7-12](#listing-7-12): A `use` statement only applies in the scope it’s in.
...
[Listing 7-13](#listing-7-13): Bringing the `add_to_waitlist` function into scope with `use`, which is unidiomatic
...
[Listing 7-14](#listing-7-14): Bringing `HashMap` into scope in an idiomatic way
...
[Listing 7-15](#listing-7-15): Bringing two types with the same name into the same scope requires using their parent modules.
...
[Listing 7-16](#listing-7-16): Renaming a type when it’s brought into scope with the `as` keyword
...
[Listing 7-17](#listing-7-17): Making a name available for any code to use from a new scope with `pub use`
...
[Listing 7-18](#listing-7-18): Specifying a nested path to bring multiple items with the same prefix into scope
...
[Listing 7-19](#listing-7-19): Two `use` statements where one is a subpath of the other
...
[Listing 7-20](#listing-7-20): Combining the paths in Listing 7-19 into one `use` statement
