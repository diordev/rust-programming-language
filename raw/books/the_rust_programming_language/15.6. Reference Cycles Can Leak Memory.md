...
source: "https://doc.rust-lang.org/stable/book/ch15-06-reference-cycles.html"
author:
published:
created: 2026-05-07
description:
...
[Listing 15-25](#listing-15-25): A cons list definition that holds a `RefCell<T>` so that we can modify what a `Cons` variant is referring to
...
[Listing 15-26](#listing-15-26): Creating a reference cycle of two `List` values pointing to each other
...
![A rectangle labeled 'a' that points to a rectangle containing the integer 5. A rectangle labeled 'b' that points to a rectangle containing the integer 10. The rectangle containing 5 points to the rectangle containing 10, and the rectangle containing 10 points back to the rectangle containing 5, creating a cycle.](https://doc.rust-lang.org/stable/book/img/trpl15-04.svg)
...
[Listing 15-27](#listing-15-27): Creating a `leaf` node with no children and a `branch` node with `leaf` as one of its children
...
[Listing 15-28](#listing-15-28): A `leaf` node with a weak reference to its parent node, `branch`
...
#### Visualizing Changes to strong\_count and weak\_count
...
[Listing 15-29](#listing-15-29): Creating `branch` in an inner scope and examining strong and weak reference counts
