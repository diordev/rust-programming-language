...
source: "https://doc.rust-lang.org/stable/book/ch18-02-trait-objects.html"
author:
published:
created: 2026-05-07
description:
...
[Listing 18-3](#listing-18-3): Definition of the `Draw` trait
...
[Listing 18-4](#listing-18-4): Definition of the `Screen` struct with a `components` field holding a vector of trait objects that implement the `Draw` trait
...
[Listing 18-5](#listing-18-5): A `run` method on `Screen` that calls the `draw` method on each component
...
[Listing 18-6](#listing-18-6): An alternate implementation of the `Screen` struct and its `run` method using generics and trait bounds
...
[Listing 18-7](#listing-18-7): A `Button` struct that implements the `Draw` trait
...
[Listing 18-8](#listing-18-8): Another crate using `gui` and implementing the `Draw` trait on a `SelectBox` struct
...
[Listing 18-9](#listing-18-9): Using trait objects to store values of different types that implement the same trait
...
[Listing 18-10](#listing-18-10): Attempting to use a type that doesn’t implement the trait object’s trait
