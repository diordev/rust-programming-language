...
source: "https://doc.rust-lang.org/stable/book/ch18-03-oo-design-patterns.html"
author:
published:
created: 2026-05-07
description:
...
[Listing 18-11](#listing-18-11): Code that demonstrates the desired behavior we want our `blog` crate to have
...
[Listing 18-12](#listing-18-12): Definition of a `Post` struct and a `new` function that creates a new `Post` instance, a `State` trait, and a `Draft` struct
...
[Listing 18-13](#listing-18-13): Implementing the `add_text` method to add text to a post’s `content`
...
[Listing 18-14](#listing-18-14): Adding a placeholder implementation for the `content` method on `Post` that always returns an empty string slice
...
[Listing 18-15](#listing-18-15): Implementing `request_review` methods on `Post` and the `State` trait
...
[Listing 18-16](#listing-18-16): Implementing the `approve` method on `Post` and the `State` trait
...
[Listing 18-17](#listing-18-17): Updating the `content` method on `Post` to delegate to a `content` method on `State`
...
We then call the `unwrap` method, which we know will never panic because we know the methods on `Post` ensure that `state` will always contain a `Some` value when those methods are done. This is one of the cases we talked about in the [“When You Have More Information Than the Compiler”](https://doc.rust-lang.org/stable/book/ch09-03-to-panic-or-not-to-panic.html#cases-in-which-you-have-more-information-than-the-compiler) section of Chapter 9 when we know that a `None` value is never possible, even though the compiler isn’t able to understand that.
...
[Listing 18-18](#listing-18-18): Adding the `content` method to the `State` trait
...
[Listing 18-19](#listing-18-19): A `Post` with a `content` method and a `DraftPost` without a `content` method
...
[Listing 18-20](#listing-18-20): A `PendingReviewPost` that gets created by calling `request_review` on `DraftPost` and an `approve` method that turns a `PendingReviewPost` into a published `Post`
...
[Listing 18-21](#listing-18-21): Modifications to `main` to use the new implementation of the blog post workflow
