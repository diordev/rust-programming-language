...
[Listing 14-1](#listing-14-1): A documentation comment for a function
...
[Listing 14-2](#listing-14-2): The documentation for the `my_crate` crate as a whole
...
![Rendered HTML documentation with a comment for the crate as a whole](https://doc.rust-lang.org/stable/book/img/trpl14-02.png)
...
[Listing 14-3](#listing-14-3): An `art` library with items organized into `kinds` and `utils` modules
...
[Listing 14-4](#listing-14-4): A crate using the `art` crate’s items with its internal structure exported
...
[Listing 14-5](#listing-14-5): Adding `pub use` statements to re-export items
...
[Listing 14-6](#listing-14-6): A program using the re-exported items from the `art` crate
