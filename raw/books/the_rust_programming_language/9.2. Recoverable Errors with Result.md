...
[Listing 9-3](#listing-9-3): Opening a file
...
[Listing 9-4](#listing-9-4): Using a `match` expression to handle the `Result` variants that might be returned
...
[Listing 9-5](#listing-9-5): Handling different kinds of errors in different ways
...
[Listing 9-6](#listing-9-6): A function that returns errors to the calling code using `match`
...
[Listing 9-7](#listing-9-7): A function that returns errors to the calling code using the `?` operator
...
[Listing 9-8](#listing-9-8): Chaining method calls after the `?` operator
...
[Listing 9-9](#listing-9-9): Using `fs::read_to_string` instead of opening and then reading the file
...
[Listing 9-10](#listing-9-10): Attempting to use the `?` in the `main` function that returns `()` won’t compile.
...
[Listing 9-11](#listing-9-11): Using the `?` operator on an `Option<T>` value
