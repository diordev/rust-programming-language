---
title: Result
source: https://rust-for-backend-developers.github.io/rust-basics/result.html
author:
published:
created: 2026-05-16
...
Как мы сказали, тип `Result` позволяет использовать любой тип для представления ошибки. Однако стандартная библиотека предоставляет трэйт [std::error::Error](https://doc.rust-lang.org/std/error/trait.Error.html), который создан специально для типов, используемых в качестве ошибки в `Result`:
...
> Note
> 
> Мы еще не разбирали API для работы с файловой системой и сделаем это только в главе [Файловая система](https://rust-for-backend-developers.github.io/dive-deeper/file-system.html). Однако если вы хотя бы раз работали с файловой системой на других языках программирования, то этот код должен выглядеть понятным:
...
Если мы сознательно хотим проигнорировать результат, то следует его явно отбросить путём присваивания [“выброшенной” переменной](https://rust-for-backend-developers.github.io/rust-basics/variables.html#discarded-variable) — `let _ =`.
