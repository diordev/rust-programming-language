---
title: Циклы
source: https://rust-for-backend-developers.github.io/rust-basics/loops.html
author:
published:
created: 2026-05-16
...
| Такой код ```rust fn main() {     let mut start = 0;     let mut sum = 0;      while {         sum += start;         start += 1;         start < 10     } {};      println!("sum: {}", sum); } ``` | работает словно ```rust fn main() {     let mut start = 0;     let mut sum = 0;      do {         sum += start;         start += 1;     } while start < 10;      println!("sum: {}", sum); } ``` |
| --- | --- |
...
> Note
> 
> Детальнее о цикле for мы поговорим в главах [Владение](https://rust-for-backend-developers.github.io/rust-basics/ownership.html) и [Итераторы](https://rust-for-backend-developers.github.io/rust-basics/iterators.html). Также в главе про [Итераторы](https://rust-for-backend-developers.github.io/rust-basics/iterators.html) мы подробнее разберём диапазоны.