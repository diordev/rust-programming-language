...
source: "https://doc.rust-lang.org/stable/book/ch12-03-improving-error-handling-and-modularity.html"
author:
published:
created: 2026-05-06
description:
...
[Listing 12-5](#listing-12-5): Extracting a `parse_config` function from `main`
...
[Listing 12-6](#listing-12-6): Refactoring `parse_config` to return an instance of a `Config` struct
...
[Listing 12-7](#listing-12-7): Changing `parse_config` into `Config::new`
...
[Listing 12-8](#listing-12-8): Adding a check for the number of arguments
...
[Listing 12-9](#listing-12-9): Returning a `Result` from `Config::build`
...
[Listing 12-10](#listing-12-10): Exiting with an error code if building a `Config` fails
...
Now that we’ve finished refactoring the configuration parsing, let’s turn to the program’s logic. As we stated in [“Separating Concerns in Binary Projects”](#separation-of-concerns-for-binary-projects), we’ll extract a function named `run` that will hold all the logic currently in the `main` function that isn’t involved with setting up configuration or handling errors. When we’re done, the `main` function will be concise and easy to verify by inspection, and we’ll be able to write tests for all the other logic.
...
[Listing 12-11](#listing-12-11): Extracting a `run` function containing the rest of the program logic
...
[Listing 12-12](#listing-12-12): Changing the `run` function to return `Result`
...
[Listing 12-13](#listing-12-13): Defining the `search` function in *src/lib.rs*
...
[Listing 12-14](#listing-12-14): Using the `minigrep` library crate’s `search` function in *src/main.rs*
