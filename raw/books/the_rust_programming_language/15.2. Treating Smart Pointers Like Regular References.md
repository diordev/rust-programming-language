...
source: "https://doc.rust-lang.org/stable/book/ch15-02-deref.html"
author:
published:
created: 2026-05-08
description:
...
[Listing 15-6](#listing-15-6): Using the dereference operator to follow a reference to an `i32` value
...
[Listing 15-7](#listing-15-7): Using the dereference operator on a `Box<i32>`
...
[Listing 15-8](#listing-15-8): Defining a `MyBox<T>` type
...
[Listing 15-9](#listing-15-9): Attempting to use `MyBox<T>` in the same way we used references and `Box<T>`
...
[Listing 15-10](#listing-15-10): Implementing `Deref` on `MyBox<T>`
...
[Listing 15-11](#listing-15-11): A `hello` function that has the parameter `name` of type `&str`
...
[Listing 15-12](#listing-15-12): Calling `hello` with a reference to a `MyBox<String>` value, which works because of deref coercion
...
[Listing 15-13](#listing-15-13): The code we would have to write if Rust didn’t have deref coercion
