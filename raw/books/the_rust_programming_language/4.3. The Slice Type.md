...
*Slices* let you reference a contiguous sequence of elements in a [collection](https://doc.rust-lang.org/stable/book/ch08-00-common-collections.html). A slice is a kind of reference, so it does not have ownership.
...
[Listing 4-7](#listing-4-7): The `first_word` function that returns a byte index value into the `String` parameter
...
[Listing 4-8](#listing-4-8): Storing the result from calling the `first_word` function and then changing the `String` contents
...
has a length value of 5 and points to byte 6 of the heap data table.](https://doc.rust-lang.org/stable/book/img/trpl04-07.svg)
...
[Listing 4-9](#listing-4-9): Improving the `first_word` function by using a string slice for the type of the `s` parameter
